...
export function checkDebarment(bidderName: string, gstin: string) {
  return mockDebarment.find(
    (d) =>
      d.status === "Active" &&
      (d.entityName.toLowerCase() === bidderName.toLowerCase() ||
        d.gstin === gstin)
  )
}
...
const mockBidders = [
  {
    id: 1,
    name: "Alton Plastic Pvt Ltd",
    gstin: "05ABNTY3290P8ZB",
    complianceScore: 92,
    verificationDepth: 88,
    riskLevel: "Low",
    status: "Recommended",
    tenderId: "TND-001",
  },
  ...
  {
    id: 5,
    name: "Deccan Industrial Supplies",
    gstin: "05ABNTY3290P8ZF",
    complianceScore: 45,
    verificationDepth: 38,
    riskLevel: "Critical",
    status: "Disqualified",
    tenderId: "TND-003",
  },
...
const mockTenders = [
  {
    id: "TND-001",
    name: "Refinery Equipment Supply",
    bidderCount: 2,
    activeRules: 1,
    status: "Open",
  },
...
const mockNotifications = [
  {
    ...
    description: "MS Corporation — awaiting response for 3 days",
    severity: "high",
  },
  {
    id: "n2",
    title: "Conditional review pending",
    description:
      "Sunrise Traders — conditional approval needs officer sign-off",
    severity: "medium",
  },
  {
    id: "n3",
    title: "Rule draft awaiting activation",
    description:
      "Pipeline Maintenance — Local Content rule still in Draft",
    severity: "low",
  },
]
...
const mockRelationships = [
  {
    bidderA: "MS Corporation",
    bidderB: "Sunrise Traders",
    sharedAttribute: "Registered Address",
    detail: "Both list 10, Veer Nariman Road, Fort",
  },
]

const mockFindings: Record<
  number,
  Array<{
    id: string
    requirement: string
...
> = {
  1: [
    {
      id: "f1",
      requirement: "GST Registration Active",
      category: "GST_REGISTRATION",
      status: "PASS",
      source: "GST Public API",
      evidenceRef: "ev-gst-001",
    },
    {
      id: "f2",
    ...
    },
    {
      ...
      category: "DEBARMENT",
      status: "PASS",
      source: "Federated Index",
      evidenceRef: "ev-debar-001",
    },
  ],
  2: [
    {
...
let mockAuditEntries = [
  ...
  },
  {
    ...
    actionType: "Verification",
    description:
      "Debarment check — no match found — Kaveri Engineering Works",
    ruleVersion: "v1.3",
    actor: "System",
    evidenceRef: "ev-005",
    bidderId: "4",
...
export async function apiFetch(path: string, options: RequestInit = {}) {
  ...
  if (USE_MOCK) {
    ...
    if (
      path.match(/^\/bidders\/\d+\/debarment-check$/) &&
      method === "GET"
    ) {
    ...
    if (
      path.startsWith("/bidders") &&
      method === "GET" &&
      !path.match(/^\/bidders\/\d+/)
    ) {
      const url = new URL(`http://localhost${path}`)
      const tenderId = url.searchParams.get("tenderId")
    ...
    if (path === "/officer/decision" && method === "POST") {
      ...
      if (bidder) {
        ...
        bidder.status =
          statusMap[body.action] ?? bidder.status

        const lastEntry =
          mockAuditEntries[mockAuditEntries.length - 1]
        ...
        const newEntry = {
          ...
          actionType: "Officer Decision",
          description: `${
            body.action.charAt(0).toUpperCase() +
            body.action.slice(1)
          } applied — ${bidder.name}${
          ...
          tenderName: "—",
          eventHash: Math.random()
            .toString(16)
            .slice(2, 18),
          previousHash:
            lastEntry?.eventHash ?? "GENESIS",
    ...
    if (
      ...
      if (
        rule.requirementType === "Turnover Threshold"
      ) {
        ...
        const actual =
          turnoverByBidder[bidder.id] ?? 0

        const required = Number(
          rule.value.replace(/[₹,\s]/g, "")
        )

        actualValue = `₹${actual.toLocaleString(
          "en-IN"
        )}`

        passed = actual >= required
      } else if (
        rule.requirementType === "MSE Preference"
      ) {
        ...
        const actual =
          preferenceByBidder[bidder.id] ?? 0
        ...
        const actual =
          localContentByBidder[bidder.id] ?? 0
    ...
    if (
      ...
      const highestVersion = versions.reduce(
        (highest, rule) => {
          const match = rule.version.match(
            /^v1\.(\d+)$/
          )
    ...
    // DEBARMENT SEARCH
    if (
      path.startsWith("/debarment/search") &&
      method === "GET"
    ) {
      const url = new URL(
        `http://localhost${path}`
    ...
    console.warn(
      `No mock defined for ${key}, returning empty object`
    )
